...
# System 5 Review desk-wide (18 Jul 2026): 10 -> 7. The old 10 sat above FTSE's median
# hourly move (~7.5pt), classifying only ~36%% of windows; 7 (~median) restores ~50%%
# classification and a healthier Morgan learning signal. Existing rows re-scored on 18 Jul.
VERDICT_THRESHOLD = 7  # 7 index pts on the 1hr window -- under = noise (NEUTRAL), over = CORRECT/WRONG


# ── Contrarian phantom log (Job 2, Gaius Commission 001) ─────────────────────
# The opposite-direction mirror of phantom_trades.csv: for every blocked signal,
# what would the OPPOSITE trade have done? DERIVED from phantom_trades.csv, so it
# reuses the phantom price checkpoints (no extra fetches), is always consistent,
# and is restart-safe. Disabled by default; a system opts in by calling
# enable_contrarian(system_name) once at startup -- this is the configurable flag
# for extending to other systems later (the machinery below is inert until then,
# so this file is safe to copy to another system verbatim).
CONTRARIAN_CSV = os.path.join(os.path.dirname(__file__), 'logs',
                              'phantom_trades_contrarian.csv')
_contrarian_lock = threading.Lock()
_contrarian_enabled = False
_contrarian_system = ''

CONTRARIAN_FIELDNAMES = [
    'timestamp_utc',     # same instant as the phantom row (UTC ISO)
    'system',            # e.g. FTSEHybrid
    'direction',         # the contrarian trade taken (opposite of the block)
    'signal_direction',  # the originally blocked signal
    'entry_price',       # same price as the phantom row
    'pnl_30min',         # contrarian-direction P&L at +30min (= -direct P&L)
    'pnl_1hr',           # contrarian-direction P&L at +1hr
    'pnl_2hr',           # contrarian-direction P&L at +2hr
    'verdict_30min',     # CORRECT (contrarian profited) / WRONG / NEUTRAL / PENDING
    'verdict_1hr',
    'verdict_2hr',
    'morgan_score',      # Morgan confidence at signal time (from the phantom row)
    'session',           # session at signal time (from the phantom row)
]
...
# ── Contrarian log helpers ───────────────────────────────────────────────────
def enable_contrarian(system_name):
    """Turn on contrarian-mirror logging for this process (call once at startup,
    e.g. in main() alongside resolve_stale_pending/start_watchdog). Builds/
    refreshes phantom_trades_contrarian.csv from existing phantom history."""
    global _contrarian_enabled, _contrarian_system
    _contrarian_enabled = True
    _contrarian_system = system_name
    logger.info("phantom_tracker: contrarian logging ENABLED for %s", system_name)
    try:
        regenerate_contrarian()
    except Exception as e:
        logger.error("phantom_tracker: initial contrarian build failed: %s", e)


def contrarian_enabled():
    return _contrarian_enabled


def _neg(v):
    """Negate a numeric string/number (opposite-direction P&L); '' for blank."""
    if v is None or v == '':
        return ''
    try:
...
def _contrarian_verdict(pnl):
    """Verdict for the TAKEN contrarian trade, from its P&L (opposite semantics to
    the stay-out verdict): CORRECT = it profited (>threshold), WRONG = it lost
    (<-threshold), NEUTRAL = small move, PENDING = not resolved yet."""
...
def _refresh_contrarian():
    """Regenerate the contrarian log if enabled -- best-effort, never raises into
    the caller (a phantom write must never fail because of the mirror)."""
    if not _contrarian_enabled:
        return
    try:
        regenerate_contrarian()
    except Exception as e:
        logger.warning("phantom_tracker: contrarian refresh failed: %s", e)


def regenerate_contrarian():
    """Rebuild phantom_trades_contrarian.csv as the exact opposite-direction mirror
    of phantom_trades.csv (contrarian P&L = -direct P&L). No-op unless contrarian
    logging is enabled. Reads the phantom log (the source of truth) so it always
    reflects the latest checkpoints and survives restarts."""
    if not _contrarian_enabled:
        return
    _ensure_csv_exists()
    with _csv_lock:
    ...
    out = []
    for r in rows:
        blocked = (r.get('direction_blocked') or '').strip()
        if blocked == 'SHORT':
            contra = 'LONG'
        elif blocked == 'LONG':
            contra = 'SHORT'
            ...
            continue
        p30 = _neg(r.get('pnl_30min'))
        p60 = _neg(r.get('pnl_1hr'))
        p120 = _neg(r.get('pnl_2hr'))
        out.append({
            'timestamp_utc':    r.get('timestamp', ''),
            'system':           _contrarian_system,
            'direction':        contra,
            'signal_direction': blocked,
            'entry_price':      r.get('price_at_decision', ''),
            'pnl_30min':        p30,
            'pnl_1hr':          p60,
            'pnl_2hr':          p120,
            'verdict_30min':    _contrarian_verdict(p30),
            'verdict_1hr':      _contrarian_verdict(p60),
            'verdict_2hr':      _contrarian_verdict(p120),
            'morgan_score':     r.get('morgan_score', ''),
            'session':          r.get('session', ''),
        })
    os.makedirs(os.path.dirname(CONTRARIAN_CSV), exist_ok=True)
    with _contrarian_lock:
        with open(CONTRARIAN_CSV, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=CONTRARIAN_FIELDNAMES)
            writer.writeheader()
            writer.writerows(out)


def get_contrarian_summary():
    """Summary of the contrarian log on the 1hr verdict horizon. Returns dict with
    decisions / correct / wrong / neutral / net_pnl (net contrarian pnl_1hr).
    Used by Chronicle and Gaius. Safe to call even when contrarian is disabled."""
    empty = {'decisions': 0, 'correct': 0, 'wrong': 0, 'neutral': 0, 'net_pnl': 0.0}
    if not os.path.exists(CONTRARIAN_CSV):
        return empty
    try:
        with _contrarian_lock:
            with open(CONTRARIAN_CSV, 'r', newline='') as f:
                rows = list(csv.DictReader(f))
    except Exception as e:
        logger.error("phantom_tracker: get_contrarian_summary error: %s", e)
        return empty
    resolved = [r for r in rows if r.get('verdict_1hr') in ('CORRECT', 'WRONG', 'NEUTRAL')]
    net = 0.0
    for r in rows:
        try:
            net += float(r.get('pnl_1hr') or 0)
        except (TypeError, ValueError):
            pass
    return {
        'decisions': len(rows),
        'correct':   sum(1 for r in resolved if r['verdict_1hr'] == 'CORRECT'),
        'wrong':     sum(1 for r in resolved if r['verdict_1hr'] == 'WRONG'),
        'neutral':   sum(1 for r in resolved if r['verdict_1hr'] == 'NEUTRAL'),
        'net_pnl':   round(net, 2),
    }
...
def record_decision(market, direction_blocked, price_at_decision,
    ...
    # Mirror this decision into the contrarian log (no-op unless enabled).
    _refresh_contrarian()
...
def resolve_stale_pending(get_historical_price_fn):
    ...
    # Keep the contrarian mirror in step with resolved phantom rows.
    _refresh_contrarian()
