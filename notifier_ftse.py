"""
FTSEHybrid AI -- notifier_ftse.py  (Percival)
...
log = logging.getLogger("FTSEHybrid.Percival")
...
def notify_trade_opened(
    ...
    _send(
        title   = "[FTSE] Trade Opened -- FTSEHybrid AI",
        message = (
...
def notify_trade_closed_win(
    ...
    _send(
        title   = "[FTSE] Trade WON -- FTSEHybrid AI",
        message = (
...
def notify_trade_closed_loss(
    ...
    _send(
        title   = "[FTSE] Trade Lost -- FTSEHybrid AI",
        message = (
            f"{direction} closed at {exit_price:,.1f}\n"
...
def notify_kill_switch_triggered(
    ...
    _send(
        title   = f"[FTSE] KILL SWITCH Tier {tier} -- FTSEHybrid AI",
        message = (
            f"{reason}\n"
...
def notify_kill_switch_reset(tier: int, wait_hours: int, capital: float) -> None:
    _send(
        title   = "[FTSE] Trading Resuming -- FTSEHybrid AI",
        message = (
...
def notify_tier3_urgent() -> None:
    _send(
        ...
        message = (
            "FTSEHybrid AI kill switch triggered 3x in 48 hours.\n"
...
def notify_system_startup(capital: float, mode: str = "PAPER") -> None:
    _send(
        title   = f"[FTSE] FTSEHybrid AI Started ({mode})",
        message = (
            f"FTSEHybrid AI is live.\n"
...
def notify_system_shutdown(capital: float) -> None:
    _send(
        title   = "[FTSE] FTSEHybrid AI Shutdown",
        message = (
            f"FTSEHybrid AI stopped cleanly.\n"
...
def notify_system_error(error_msg: str) -> None:
    _send(
        title   = "[FTSE] System Error -- FTSEHybrid AI",
