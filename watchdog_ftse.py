"""
FTSEHybrid AI -- watchdog_ftse.py  (Galahad)
Monitors main_ftsehybrid.py and automatically restarts on crash or freeze.
...
MAIN_SCRIPT   = BASE_DIR / "main_ftsehybrid.py"
LOG_FILE      = BASE_DIR / "logs" / "watchdog_ftse.log"
...
log = logging.getLogger("FTSEHybrid.Galahad")
    ...
    try:
        requests.post(
            ...
            data={"token": _PO_TOKEN, "user": _PO_USER,
                  "title": f"[FTSE] {title}", "message": message, "priority": priority},
            timeout=5,
        )
    except Exception:
        pass
...
class Watchdog:
    ...
    def _launch(self) -> None:
        ...
        log.info("FTSEHybrid AI started (PID %d)", self._proc.pid)
    ...
    def _check_restart_flag(self) -> bool:
        ...
        if RESTART_FLAG.exists():
            ...
            log.info("restart.flag detected -- restarting FTSEHybrid AI")
            return True
        return False
    ...
    def _do_restart(self, reason: str) -> bool:
        ...
        if self._alive():
            ...
            log.info("FTSEHybrid AI restarted (PID %d)", self._proc.pid)
    ...
    def _handle_limit_exceeded(self) -> None:
        self._prune_restarts()
        msg = (
            f"FTSEHybrid AI crashed {MAX_RESTARTS}x in 1 hour. "
    ...
    def _maybe_heartbeat(self) -> None:
        ...
        log.info(
            "Galahad heartbeat -- FTSEHybrid AI running for %.1f hours",
            uptime_h,
        )
        self._heartbeat_at = time.monotonic()
    ...
    def run(self) -> None:
        log.info("=" * 60)
        log.info("  FTSEHybrid AI -- Galahad Watchdog")
        ...
        try:
            ...
            log.error("FATAL: Could not start FTSEHybrid AI: %s", exc)
            return
    ...
    def shutdown(self) -> None:
        ...
        _push("Shutdown", "FTSEHybrid AI stopped cleanly.")
