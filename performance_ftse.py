"""
FTSEHybrid AI -- performance_ftse.py  (Morgan)
...
log = logging.getLogger("FTSEHybrid.Morgan")
...
def _compute_confidence(df: pd.DataFrame) -> dict:
    ...
    if total >= 10:
        ...
        if "session_phase" in df.columns:
            for phase in ["MORNING_PRIME", "AFTERNOON"]:
...
def _compute_direction_session_stats(df: pd.DataFrame) -> dict:
    ...
    session_stats = {}
    if "session_phase" in df.columns:
        for s in ["MORNING_PRIME", "AFTERNOON", "LUNCH_LULL"]:
...
def generate_milestone_review(trades_log: Path, milestone_num: int) -> None:
    ...
    lines = [
        "=" * 60,
        f"FTSEHybrid AI -- Arthur Milestone Review #{milestone_num}",
