...
def run_candle_tick(
    ...
    stanley.fill_post_exit_prices(ftse_price, now_utc)
    ...
    if stanley.in_trade:
        ...
        if action == "EXIT":
            # Gaius Commission 012: capture the indicator snapshot + Arthur's exit
            # confidence so we can later judge whether the exit was skill or premature.
            _emeta = _build_exit_meta(ind_1d, ind_1h, ind_5m, decision)
            _close_trade(stanley, account, ig, ftse_price, "ARTHUR_EXIT", exit_meta=_emeta)
        else:
...
def _apply_confidence_lift() -> None:
    ...
    try:
        ...
        if data.get("reset_gating"):
            ...
            prior = performance_ftse.get_perf_dashboard_dict().get("confidence_score")
            performance_ftse.reset_to_50(reason=reason)
