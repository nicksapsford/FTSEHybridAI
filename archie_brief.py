...
def _guinevere_kind(system_name):
    """Which Guinevere module a system runs. Detected by whether a guinevere_news
    module is importable in-process (Gold/Oil/Gas AND FTSE, which gained a working
    news module on 18-19 Jul 2026); otherwise calendar-only (US, Crypto), else none.

    Fix (21 Jul 2026): FTSE was hardcoded to 'calendar' even though guinevere_news
    is present and wired, so its Archie Brief wrongly showed "Calendar only". The
    import check now reflects the real module state and cannot drift again."""
    try:
        import guinevere_news  # noqa: F401 -- presence is the signal
        return "news"
    except Exception:
        pass
    n = (system_name or "").lower()
    if any(x in n for x in ("gold", "oil", "gas", "ftse")):
        return "news"
    if any(x in n for x in ("us", "crypto")):
        return "calendar"
    return "none"
