"""
FTSEHybrid AI -- backtest_ftse.py

FTSE 100 spread betting backtest engine using the proven TrendSurfer indicator suite.
...
Output:
  logs/backtest_ftse_results.txt  -- best configuration full detail
  logs/backtest_ftse_sweep.csv    -- all 12 combinations
  logs/backtest_ftse_trades.csv   -- all trades across all combos
"""
...
log = logging.getLogger("FTSEHybrid.Backtest")

# ── Constants ──────────────────────────────────────────────────────────────────

STARTING_CAPITAL_GBP = 500.0
MAX_RISK_PER_TRADE   = 10.0        # £10 = 2% of £500 account
IG_SPREAD_POINTS     = 1.0         # IG Markets FTSE 100 spread (1 point)
...
TRADEABLE_PHASES = {MORNING_PRIME, AFTERNOON, LUNCH_LULL}
...
def get_session_phase(ts_uk) -> str:
    """Return the FTSE trading session phase for a UK-local pandas Timestamp."""
    if ts_uk.weekday() >= 5:
        return CLOSED
    ...
    elif t < 720:   return MORNING_PRIME # 08:15 – 11:59
    elif t < 810:   return LUNCH_LULL    # 12:00 – 13:29
...
@dataclass
class FTSETrade:
    """One FTSE 100 spread bet. P&L = pnl_pts × stake."""

    direction:   str
    entry_price: float     # FTSE index level (e.g. 8215.0)
...
def _fetch_ftse_fine() -> tuple:
    """Fetch finest FTSE intraday data with fallbacks. Returns (df, resolution_str)."""
    for ticker, interval, period, min_bars, label in [
        (FTSE_TICKER, "5m",  "60d", 200, "^FTSE 5m"),
        (FTSE_TICKER, "15m", "60d", 100, "^FTSE 15m (FALLBACK -- 5m unavailable)"),
        ("EWU",       "5m",  "60d",  50, "EWU 5m (PROXY -- ^FTSE 5m/15m unavailable)"),
    ]:
    ...
    raise RuntimeError("Could not obtain usable FTSE intraday data from Yahoo Finance")
...
def fetch_historical_data() -> tuple:
    ...
    log.info("  ^FTSE 1d (2y)...")
    df_1d = add_indicators(_yf_download(FTSE_TICKER, "1d", "2y"))
    log.info("    %d candles  [%s -> %s]",
             len(df_1d),
             df_1d.index[0].strftime("%Y-%m-%d"),
             df_1d.index[-1].strftime("%Y-%m-%d"))

    log.info("  ^FTSE 1h (90d)...")
    df_1h = add_indicators(_yf_download(FTSE_TICKER, "1h", "90d"))
    log.info("    %d candles  [%s -> %s]",
             len(df_1h),
             df_1h.index[0].strftime("%Y-%m-%d %H:%M"),
             df_1h.index[-1].strftime("%Y-%m-%d %H:%M"))

    log.info("  FTSE finest intraday...")
    df_fine, resolution = _fetch_ftse_fine()
...
def run_single_backtest(
    ...
    current_trade: Optional[FTSETrade] = None
    completed      = []
    ...
    for ts, bar in df_fine.iterrows():
        ...
        if phase not in TRADEABLE_PHASES:
            continue
        if phase == LUNCH_LULL and lunch_restricted:
            continue
...
def _compute_stats(completed: list, final_capital: float,
    ...
    by_session = {
        ...
        for ph in (MORNING_PRIME, AFTERNOON, LUNCH_LULL)
    }
...
def _save_sweep_csv(all_results: list) -> None:
    path = LOGS_DIR / "backtest_ftse_sweep.csv"
    fields = [
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for s, _ in all_results:
            ec  = s["exit_counts"]
            bs  = s["by_session"]
            mp  = bs.get(MORNING_PRIME, {"count": 0, "win_rate": 0.0})
            af  = bs.get(AFTERNOON,     {"count": 0, "win_rate": 0.0})
            ll  = bs.get(LUNCH_LULL,    {"count": 0, "win_rate": 0.0})
            w.writerow({
                "combo":                  s["combo_num"],
                "label":                  s["label"],
                "stop_pts":               s["stop_pts"],
...
def _save_trades_csv(all_results: list) -> None:
    path = LOGS_DIR / "backtest_ftse_trades.csv"
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow([
            "combo", "label", "stop_pts", "lunch_restricted", "gbpusd_filter",
...
def _save_results_txt(best_stats: dict, best_trades: list,
    ...
    path     = LOGS_DIR / "backtest_ftse_results.txt"
    ...
    lines = [
        "=" * 72,
        "  FTSEHybrid AI -- FTSE 100 Spread Betting Backtest",
        f"  Generated  : {now_str}",
        f"  Data       : {resolution}",
    ...
    for ph, label in [(MORNING_PRIME, "Morning  8:15-12:00"),
                      (AFTERNOON,     "Afternoon 13:30-15:30"),
                      (LUNCH_LULL,    "Lunch    12:00-13:30")]:
...
def _print_sweep_table(all_results: list, df_fine: pd.DataFrame, resolution: str) -> None:
    ...
    print()
    print(sep)
    print("  FTSEHYBRID AI -- FTSE 100 SPREAD BETTING  |  PARAMETER SWEEP")
...
# ── Terminal: FTSE analysis questions ─────────────────────────────────────────
...
def _print_analysis(all_results: list, df_fine: pd.DataFrame) -> None:
    ...
    mp = bs.get(MORNING_PRIME, {"count": 0, "win_rate": 0.0, "net_pnl": 0.0})
    af = bs.get(AFTERNOON,     {"count": 0, "win_rate": 0.0, "net_pnl": 0.0})
    ll = bs.get(LUNCH_LULL,    {"count": 0, "win_rate": 0.0, "net_pnl": 0.0})
    ...
    print()
    print(sep)
    print("  FTSE-SPECIFIC ANALYSIS -- 6 KEY QUESTIONS")
    print(sep)

    # Q1
    print()
    print("  Q1: SIGNALS PER WEEK -- FTSE 100 vs BTC/GBP vs ETH/GBP")
    print(div)
    print(f"  FTSE 100 best config : {best_stats['trades_per_week']:.1f} trades/week"
    ...
    if diff < BTC_STRICT["trades_pw"]:
        print("  >> FTSE generates FEWER signals than either crypto.")
        print("     This is expected: FTSE trades only ~8.25h/day vs crypto 24h.")
        print("     Signal density per trading hour may be comparable.")
    elif diff < ETH_STRICT["trades_pw"]:
        print("  >> FTSE signal frequency sits between BTC and ETH.")
    else:
        print("  >> FTSE generates MORE signals than crypto -- note 8.25h day vs 24h.")
    ...
    print(div)
    approx_ftse = 8000   # approximate FTSE 100 level for % context
    for pts in TS_OPTIONS:
        s = best_by_stop[pts]
        pct_of_idx = pts / approx_ftse * 100
        print(f"  {pts:>2}pt stop: best net GBP {s['net_pnl']:>+.2f}"
              f"  ({s['total']} trades, {s['win_rate']:.0f}% WR)"
              f"  = {pct_of_idx:.3f}% of FTSE at 8,000")
    print(f"  >> OPTIMAL: {best_stop_pts} points"
          f"  (net GBP {best_stop_stats['net_pnl']:>+.2f}  /"
          f"  {best_stop_pts/approx_ftse*100:.3f}% of index at 8,000)")

    # Q6
    print()
    print("  Q6: IS THE STRATEGY VIABLE ON FTSE 100?")
    print(div)
    ...
    if answer == "YES":
        ...
        print(f"     Strategy not profitable on this FTSE sample.")
...
def _print_comparison_table(best_stats: dict) -> None:
    ...
    def row(label, btc_v, eth_v, ftse_v):
        print(f"  {label:<40} {str(btc_v):>{W}} {str(eth_v):>{W}} {str(ftse_v):>{W}}")

    def div():
        print("  " + "-" * (38 + W * 3 + 2))

    ftse_ann = best_stats["ann_return"]

    print()
    print(sep)
    print("  PERFORMANCE COMPARISON: BTC/GBP vs ETH/GBP vs FTSE 100")
    print("  (all: STRICT mode, best configuration each)")
    print(sep)
    print(f"  {'':40} {'BTC/GBP':>{W}} {'ETH/GBP':>{W}} {'FTSE 100':>{W}}")
    div()
    ...
    print(f"  {ftse_ann:>+.0f}% gross on FTSE = {ftse_ann:>+.0f}% kept in full.")
    print(f"  Same gross return on BTC/ETH ~ {ftse_ann*0.76:>+.0f}% after estimated CGT.")
    if btc_after_tax > ftse_ann > 0:
        gap = btc_after_tax - ftse_ann
        print(f"  BTC leads after tax by ~{gap:.0f}pp, but FTSE is tax-free and lower risk.")
    elif ftse_ann > btc_after_tax:
        print(f"  FTSE BEATS crypto on after-tax basis by ~{ftse_ann - btc_after_tax:.0f}pp.")
    print(sep)
...
def _print_verdict(best_stats: dict, all_results: list) -> None:
    ...
    print()
    print(sep)
    print("  FTSEHYBRID AI -- HONEST VERDICT")
    print(sep)

    print()
    print("  1. DOES THE STRATEGY WORK ON FTSE 100?")
    print(div)
    if viable and s["ann_return"] > 20:
        verdict1 = "YES -- Strategy is profitable on FTSE 100."
    elif viable:
    ...
    print(f"  Trailing stop  : {s['stop_pts']} points"
          f"  (= {s['stop_pts']/8000*100:.3f}% of FTSE at 8,000)")
    ...
    print()
    print("  3. FTSE-SPECIFIC FACTORS THAT MOST IMPROVED RESULTS")
    print(div)
    if filter_gain > 0:
    ...
    print(f"  c) Points-based stop ({s['stop_pts']}pt) suited to FTSE intraday ATR")
    ...
    print()
    print("  5. WORTH BUILDING AS FTSEHYBRID AI?")
    print(div)
    if s["net_pnl"] > 0:
        ...
        print("     2. Reviewing whether FTSE 100 5m data from Yahoo Finance is complete")
        print("     3. Adjusting indicator parameters for FTSE's lower volatility profile")

    print()
    print(sep)
    print("  OUTPUT FILES:")
    print("  logs/backtest_ftse_results.txt  -- best configuration full detail")
    print("  logs/backtest_ftse_sweep.csv    -- all 12 combinations")
    print("  logs/backtest_ftse_trades.csv   -- every trade across all combos")
    print(sep)
...
def _print_sp500_note(df_sp500: pd.DataFrame, df_1d: pd.DataFrame) -> None:
    """Quick FTSE / S&P 500 daily correlation note (context only)."""
    ...
    try:
        ...
        if len(combined) < 10:
            return
        corr = combined["ftse"].pct_change().corr(combined["sp500"].pct_change())
        print()
        print(f"  S&P 500 context: FTSE 100 / S&P 500 daily return correlation = {corr:.2f}")
        if corr > 0.6:
            print(f"  >> Strong positive correlation ({corr:.2f}) -- US session direction"
                  " is meaningful context for FTSE morning gaps.")
        elif corr > 0.3:
...
def run_backtest() -> None:
    ...
    log.info("FTSEHybrid AI -- FTSE 100 Spread Betting Backtest")
    ...
    log.info("  logs/backtest_ftse_results.txt")
    log.info("  logs/backtest_ftse_sweep.csv")
    log.info("  logs/backtest_ftse_trades.csv")
