"""
FTSEHybrid AI -- dashboard_ftse.py
Two-page browser dashboard at http://localhost:5042
...
log = logging.getLogger("FTSEHybrid.Dashboard")
# ALBION STANDING RULE: all log timestamps are UTC (never BST/local). See main_ftsehybrid.py.
...
def load_trades() -> list:
    """Load all FTSE trades from CSV, most recent first."""
...
<title>FTSEHybrid A.I. &mdash; FTSE 100</title>
<style>
:root{
  --bg:#0d0d0d;--bg2:#141414;--bg3:#1e1e1e;--border:#2a2a2a;
  --gold:#4169E1;--green:#2ecc71;--red:#e74c3c;--amber:#f39c12;
...
.nav-btn{background:rgba(65,105,225,0.15);border:1px solid var(--gold);color:var(--gold);padding:4px 12px;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer;letter-spacing:0.3px;transition:background 0.15s;}
.nav-btn:hover{background:rgba(65,105,225,0.32);}
...
.btn-cancel:hover {background:rgba(65,105,225,0.15);}
...
.phase-MORNING_PRIME{background:rgba(46,204,113,0.12);color:var(--green);}
.phase-AFTERNOON    {background:rgba(52,152,219,0.15);color:#3498db;}
.phase-LUNCH_LULL   {background:rgba(243,156,18,0.12);color:var(--amber);}
...
.dec-hold {background:rgba(65,105,225,0.1);color:var(--gold); border:2px solid var(--gold);}
...
<div class="header">
  <div class="header-brand">
    <div>
      <div class="logo">FTSE<span>TRADER</span> A.I.<span class="version-tag">__VERSION_STRING__</span></div>
      <div class="subtitle">FTSE 100 Spread Betting &mdash; Capital.com</div>
    </div>
  </div>
  <div class="header-price">
    <div class="hdr-price-lbl">FTSE 100</div>
...
<div id="page1" class="page-wrap">
  <div class="main" id="main-grid">
    <div style="grid-column:1/-1;color:var(--muted);padding:40px;text-align:center">Loading FTSEHybrid AI...</div>
  </div>
</div>
...
<div id="page2" class="page-wrap" style="display:none;">
  <div class="p2-content">
    ...
    <div class="p2-card" id="p2-trades">
      <div class="card-title">FTSE Trade History</div>
      <div style="color:var(--muted);font-size:11px;">Loading...</div>
    </div>
  </div>
</div>
...
function confirmShutdown(){
  fetch('/api/shutdown', {method:'POST'})
    .then(function(){
      document.body.innerHTML = '<div style="display:flex;height:100vh;align-items:center;justify-content:center;background:#0d0d0d;color:#4169E1;font-family:monospace;font-size:18px;">FTSEHybrid AI shut down. You may close this window.</div>';
    })
...
function renderNewsCard(n){
  ...
  if(!n){
    return '<div class="card" id="newsCard" style="flex-shrink:0">' + title +
      '<div style="color:var(--muted);font-size:11px;">Loading FTSE news...</div></div>';
  }
  var sent  = n.sentiment || 'NEUTRAL';
  var scls  = sent === 'BULLISH' ? 'bull' : sent === 'BEARISH' ? 'bear' : 'neut';
  var score = (n.score === undefined || n.score === null) ? 0 : n.score;
...
function renderPage1(d){
  ...
  if(hdrEl){ hdrEl.textContent = (d.ftse_level||0).toLocaleString('en-GB',{minimumFractionDigits:1}); }
  ...
  function buildPosHTML(p, currentPrice){
    ...
    // Floating P&L computed live from the current FTSE price (serialised pnl_* are "None" while OPEN)
...
function renderPage2(d){
  ...
  if(total === 0){
    ...
    if(sesKeys.length > 0){
      var sesLabel = {'MORNING_PRIME':'Morning','AFTERNOON':'Afternoon','LUNCH_LULL':'Lunch Lull'};
  ...
  document.getElementById('p2-trades').innerHTML =
    '<div class="card-title">FTSE Trade History</div>' + tradeHTML;
}
...
def api_archie_brief():
    ...
    try:
        ...
        state = get_state()
    txt = archie_brief.build_system_brief(state, "FTSEHybrid", "FTSE 100", str(LOG_DIR))
...
def api_state():
    ...
    payload = {
        ...
        "phase":           s.get("phase", "--"),
        "ftse_level":      s.get("ftse_level", 0.0),
...
@app.route("/api/news")
def api_news():
    """Guinevere FTSE news sentiment (Part 5) + UK-event caution window + macro flag."""
    try:
        data = guinevere_news.fetch_ftse_sentiment() or {}
        ...
        try:
            cau_active, cau_reason = guinevere_news.get_ftse_calendar_status()
        except Exception:
...
if __name__ == "__main__":
    ...
    log.info("FTSEHybrid AI Dashboard starting on http://localhost:%d", PORT)
